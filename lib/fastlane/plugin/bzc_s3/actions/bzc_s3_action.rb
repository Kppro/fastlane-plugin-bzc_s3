  ...
  module Actions
    ...
    class BzcS3Action < Action
      ...
      def self.upload_ipa(s3_client, params, s3_region, s3_access_key, s3_secret_access_key, s3_bucket, ipa_file, dsym_file, s3_path, acl)
        ...
        ipa_url = self.upload_file(s3_client, s3_bucket, app_directory, ipa_file_name, ipa_file_data, acl, nil)
        ...
        if dsym_file
          ...
          dsym_url = self.upload_file(s3_client, s3_bucket, app_directory, dsym_file_name, dsym_file_data, acl, nil)
        ...
        # grabs module
        eth = Fastlane::Helper::BzcS3Helper
        ...
        if plist_template_path && File.exist?(plist_template_path)
          ...
          puts "2 - #{Helper.gem_path('fastlane-plugin-bzc_s3')}"
        ...
        #
        #####################################
        plist_url = self.upload_file(s3_client, s3_bucket, app_directory, plist_file_name, plist_render, acl, nil)
        ...
        #
        #####################################
        html_url = self.upload_file(s3_client, s3_bucket, app_directory, html_file_name, html_render, acl, nil)
        version_url = self.upload_file(s3_client, s3_bucket, app_directory, version_file_name, version_render, acl, nil)
      ...
      def self.upload_file(s3_client, bucket_name, app_directory, file_name, file_data, acl, content_type)
        content_type ||= "text/html"
        
        puts "====> upload_file with content type #{content_type}"
